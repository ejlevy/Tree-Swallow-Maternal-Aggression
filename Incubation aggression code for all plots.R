...
## Combined into one
p1 <- ggplot(fig, aes(x=reorder(Trait, Order), y=vSBN, 
                      fill = ifelse(vSBN>0 & Order > 1, "#B538A2", 
                                    ifelse(vSBN<0 & Order > 1, "#F57340",
                                          ifelse(vSBN>0 & Order==1,"#F5B69A", "#B77DAC"))))) +
  scale_fill_manual(values = c("#F57340", "#B77DAC", "#B538A2", "#F5B69A")) +
...
p2 <- ggplot(fig, aes(x=reorder(Trait, Order), y=dSBN, 
                      fill = ifelse(dSBN>0 & Order > 1, "#B538A2", 
                                    ifelse(dSBN<0 & Order > 1, "#F57340",
                                           ifelse(dSBN>0 & Order==1,"#F5B69A", "#B77DAC"))))) +
  geom_col(position = "stack") +
  scale_fill_manual(values = c("#F57340", "#B77DAC", "#B538A2", "#F5B69A")) +
  ylim(-225, 225) + 
  geom_hline(aes(yintercept = 0)) +
...
go <- read_xlsx("Data frames for figures.xlsx", sheet = 4)
...
go %>% 
  ...
  theme(panel.background = element_blank(), 
        ...
        axis.ticks.y = element_blank())
ggsave("vSBN 5min Positive LFC specific GO terms.jpeg", height = 4, width = 6.5, units = "in")
ggsave("vSBN 5min Positive LFC specific GO terms.eps", height = 4, width = 6.5, units = "in")
ggsave("vSBN 5min Positive LFC specific GO terms.pdf", height = 4, width = 6.5, units = "in")
...
go %>% 
  ...
  theme(panel.background = element_blank(), 
        ...
        axis.text.x = element_text(size = 10, colour = "black"),
        axis.ticks.y = element_blank())
ggsave("vSBN 5min Negative LFC all specific GO terms.jpeg", height = 3, width = 6.15, units = "in")
ggsave("vSBN 5min Negative LFC all specific GO terms.eps", height = 3, width = 6.15, units = "in")
ggsave("vSBN 5min Negative LFC all specific GO terms.pdf", height = 3, width = 6.15, units = "in")
...
ge_agg_long <- read_xlsx("Data frames for figures.xlsx", sheet = 5)
...
# Overlapping DEGs for aggression AND uncorrected DEGs for treatment (Figure 5) --------
# A version that highlights the overlapping genes between aggression and treatement 

shared <- read_xlsx("Data frames for figures.xlsx", sheet = 6)
...
all_agg_DEG <- read_xlsx("Data frames for figures.xlsx", sheet = 7)
